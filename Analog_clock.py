...
def update_clock():
    ...
    for i in range(12):
        ...
        if i==0:
             canvas.create_text(x,y-10,text=str(i+12),font=("Calibri",12))
        else:
            canvas.create_text(x,y,text=str(i),font=("Calibri",12))
    for i in range(60):
    ...
    canvas.create_line(WIDTH/2,HEIGHT/2,hour_x,hour_y,fill="green",width=6)

    minute_angle=(min+sec/60)*math.pi/30-math.pi/2
    ...
    sec_x=WIDTH/2+0.6*WIDTH/2*math.cos(second_angle)
    sec_y=HEIGHT/2+0.6*HEIGHT/2*math.sin(second_angle)
    canvas.create_line(WIDTH/2, HEIGHT/2,sec_x, sec_y, fill="red", width=2)
canvas.after(10000,update_clock())
update_clock()
root.mainloop()
